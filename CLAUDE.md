...
- **Sistemática de Testes de Estoque (Camadas 1-3) — implementada 2026-05-21, 14/17 cenários passando (82.4%) — atualizado 2026-05-22 round 2.** Pirâmide completa: unit (vitest, já existia) + integration (vitest config separado `vitest.integration.config.ts` rodando contra staging) + scenarios (17 fluxos ponta-a-ponta via HTTP em `/api/wms/*`, com runner em `scripts/wms/cenarios/run-all.ts`). 7 invariantes globais property-based rodam ao fim de todo cenário (`I1..I7` — ver `scripts/wms/cenarios/README.md`). Stubs PrintNode + ML criados (`src/lib/printnode-stub.ts`, `src/lib/ml-stub.ts`); Tiny reaproveita stub existente. Migration `20260521_test_harness_rpc.sql` adiciona `wms_truncate_operacional()`. Rodar: `npm run scenarios`. Spec: `docs/superpowers/specs/2026-05-21-sistematica-testes-estoque-design.md`. Plano original: `docs/superpowers/plans/2026-05-21-sistematica-testes-estoque.md`. Plano de destravo: `docs/superpowers/plans/2026-05-21-destravar-cenarios-estoque.md`. Smoke report: `docs/superpowers/smoke-2026-05-21-cenarios-estoque.md`. **Bugs prod fixados no round 2 (2026-05-22):** (a) migration `20260522_add_validacao_oc_status.sql` adiciona `validacao_oc` ao CHECK constraint de `siso_pedidos.status_separacao` — antes o worker fazia UPDATE silenciosamente rejeitado pelo Postgres (commit `7576546`); (b) `vendas/criar` em baixa_direta usa `randomUUID()` em `origem_id` e preserva `MAN-...` em `origem_detalhes.pedido_id_manual` (mesmo padrão de bug do commit `fe1a849`, commit `7b1b960`); (c) validação defensiva uuid em `inserirMovimentacao` via `assertUuidLike` em 12 campos — vai gritar com mensagem clara em vez de `[object Object]` quando os call sites latentes de `execution-worker-wms.ts`, `cutover.ts` (Plano 6) e `devolucoes.ts` forem ativados (commit `f8ae05f`). **Follow-ups** (3 cenários ainda falhos, todos no harness/scripts dos cenários, não bugs prod): cenário 02 espera Plano 6 (cutover lógico WMS-first) ou refresh de seed; cenários 03 e 05 são ajustes triviais nos scripts (cenário 03 envia payload obsoleto pra `/compras/comprar`, cenário 05 espera `aguardando_separacao` mas estado canônico correto agora é `validacao_oc`). Validação defensiva uuid catchá 3 sites latentes (execution-worker-wms.ts, cutover.ts, devolucoes.ts) — fixar antes do Plano 6 cutover.
