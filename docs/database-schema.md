...
| 2026-06-10 | `20260610_cron_tiny_polling.sql` | Cron pg_cron `wms_tiny_polling_fallback` (*/10min) → `net.http_get` em `/api/wms/tiny/polling` (vault `worker_secret`, timeout 120s). Polling fallback do webhook Tiny: pedidos aprovados/cancelados + NFs autorizadas, janela 7 dias. |
