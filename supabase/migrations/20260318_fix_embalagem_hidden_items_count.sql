-- Fix: siso_processar_bip_embalagem must exclude 'indisponivel' items
-- (in addition to 'cancelado') from both item search and pending count.
-- These items are hidden from the packing UI but were still being counted,
-- preventing pedido_completo from ever being true.
