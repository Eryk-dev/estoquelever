...
- Resets all items: `separacao_marcado=false`, `separacao_marcado_em=null`, `separacao_parcial=false`, `quantidade_pega=null`, `parcial_motivo/em/por=null`, `mov_saida_id=null`, `mov_ajuste_loc_zerou_id=null`, `quantidade_bipada=0`, `bipado_completo=false` (fix 2026-06-12: sem zerar o bip de embalagem, o pedido re-separado caía na aba Separados já "Bipado" e a etiqueta não disparava)
...
| `{ status: 'realocado', realocacoes: [...] }` | Cascade criou linhas novas (mesmo galpão) pra cobrir o residual. **Misto (2026-06-12):** se parte dos itens ficou sem cobertura de sistema, esses transitam pra `validacao_oc` (pick OC) e a resposta inclui `itens_enviados_validacao_oc` / `pedidos_enviados_validacao_oc`. | ambos |
...
| `{ status: 'enviado_validacao_oc', tem_em_outro_galpao: false, itens_atualizados, pedidos_atualizados }` | **2026-06-12:** cascade esgotou e nenhum galpão tem saldo de sistema → itens transitam pra `validacao_oc` (pick OC) pra busca física antes de comprar (loc cadastrada pode estar errada; só "esgotado" no pick OC → Compras). Itens viram `compra_status='oc_pendente'`. | item (loc_zerou) |
...
**Query Params:**
- `tab`: "comprar" | "receber" | "historico" | "counts" (default: "comprar")
- `counts`: retorna só `{ counts }` — usado pelo polling de badges dos tabs da página /wms/compras
...
```json
{
  ...
  "fornecedores": [
    {
      "fornecedor": "string",
      "galpao_nome": "string | null (manuais resolvem via join siso_galpoes)",
      "documentos": [
        {
          "origem": "oc" | "manual",
