...
-- Corpo IDÊNTICO ao LIVE (20260607_inserir_mov_custo_guards.sql, que já trazia
-- os guards P108/P110 — fetch via pg_get_functiondef) + 3 adições marcadas
-- '-- +P072'. ANTES de qualquer mutação: se já existe mov com essa key, retorna
-- a mov existente (no-op idempotente) — fecha o duplo-pick sem-reserva sob
-- concorrência. NADA mais muda: whitelist de custo, guard P108 (entrada custo 0),
-- reversão P110 no estorno, cast ::wms_motivo_categoria_enum, colunas
-- custo_medio_anterior/posterior e a fórmula ponderada são copiadas verbatim.
BEGIN;

-- Dropa o overload de 22 args (com p_motivo_categoria, SEM p_idempotency_key)
-- pra evitar ambiguidade no PostgREST.
