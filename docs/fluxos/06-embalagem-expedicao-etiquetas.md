...
1. **`preCriarAgrupamentosEmLote(pedidoIds[])`** (`agrupamento-service.ts:46-108`) — recria agrupamentos para pedidos que perderam fase 1 (e.g. NF chegou tarde, restart durante fase 1, ou backfill). Usa o mesmo `siso_claim_pedidos_para_agrupamento`. Para cada pedido, cria agrupamento, conclui, descobre expedição, **e nesta passada baixa+cacheia o ZPL** via `obterEtiquetasExpedicao` + `baixarZpl`.
