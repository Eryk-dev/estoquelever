...
| 9 | **encaixotamento por dia** (etapa pós-separação): operador bipa SKU/EAN + qty → sistema deposita nas caixas = DIA de `prazo_envio` dos pedidos futura `separado`, FIFO por prazo (mais urgente 1º); mostra todas as caixas de uma vez; rastreia no banco (`quantidade_encaixotada` por item, `encaixotado_em` no pedido). NÃO muda `status_separacao`. Acesso por **botão "Encaixotar" na aba Separados** da sep. futura (NÃO é item de sidebar — mesmo padrão do "Conferir" → conferência); sub-tela `/wms/separacao/encaixotamento` reusa `HandheldScan` + fila serial da conferência. **5ª aba "Encaixotados"** (só futura, aba virtual: status `separado` + `encaixotado_em IS NOT NULL`) move o pedido pronto pra fora da fila "Separados" (= `encaixotado_em IS NULL`) — counts separados via 1 HEAD-count extra na rota (sem mexer na RPC `wms_separacao_counts`) | `migrations/20260624d_encaixotamento.sql` (RPCs `wms_encaixotar_atomico`/`wms_desencaixotar_atomico`), `api/wms/encaixotamento/{route,bipar,desfazer}`, `app/wms/separacao/encaixotamento/page.tsx`, botão em `app/wms/separacao/page.tsx` (`PrimaryTabActions`) |
