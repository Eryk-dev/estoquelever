-- D.1.2 — RPC wms_inserir_movimentacao aceita p_motivo_categoria.
--
-- Reaproveita corpo de 20260526_custo_medio_ajuste_manual.sql; adiciona
-- 1 parâmetro (DEFAULT NULL — preserva compat com callers que ainda não
-- passam) + grava o valor na coluna motivo_categoria. Nenhuma outra
-- lógica (custo médio, saldo, reservado) muda.

BEGIN;

-- Dropa overload anterior (21 args, sem p_motivo_categoria) pra evitar
-- ambiguidade no PostgREST quando caller passa todos args sem o novo.
-- Ver 20260512_wms_drop_old_inserir_mov_overload.sql pra padrão similar.
DROP FUNCTION IF EXISTS public.wms_inserir_movimentacao(
  uuid, uuid, uuid, character, numeric, text, text, jsonb, uuid,
  timestamptz, uuid, uuid, uuid, uuid, uuid, text, text, text, uuid, text, numeric
);
