...
- **Quadro de Tarefas Pendentes na home `/wms` — implementado em 2026-05-21, refatorado pra kanban em 2026-05-21.** Bloco no topo da home com 4 seções fixas (sempre presentes; quando vazias aparecem como placeholder dim/dashed em vez de sumir). **Pipeline do pedido** (Aprovação · Separação · Embalagem) continua agregado em 3 cards de contador. **Guarda · Inventário · Compras** viraram kanban detalhado: cada pendência de guarda é um card individual (SKU + qty + status + avatar + tempo na fila, truncado em 50 por seção), cada ciclo de inventário é um card com barra de progresso (`locs_contadas / locs_total`), e Compras agrupa por fornecedor (1 card por fornecedor com `a_comprar / a_receber`). Endpoint: `GET /api/wms/dashboard-tarefas?galpao_id?` — retorna `guarda.itens[]`, `inventario.ciclos[]` e `compras.fornecedores[]` além dos contadores. Realtime via Supabase em 5 tabelas (`siso_pedidos`, `siso_wms_pendencias_guarda`, `siso_inventario_sessoes`, `siso_inventario_operadores`, `siso_pedido_itens`). Spec: `docs/superpowers/specs/2026-05-21-quadro-tarefas-home-design.md`. Plano: `docs/superpowers/plans/2026-05-21-quadro-tarefas-home.md`.
