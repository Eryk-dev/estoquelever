-- Revert unify_embalagem_confirmation after production regression.
-- Restore the previous 3-argument bip RPC and remove the new item-level RPC.

DROP FUNCTION IF EXISTS siso_processar_bip_embalagem(text, uuid, integer, text[]);
DROP FUNCTION IF EXISTS siso_processar_item_embalagem(uuid, integer);
