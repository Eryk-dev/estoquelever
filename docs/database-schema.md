...
| `compra_preco_unitario` | numeric(14,4) | YES | | Preço unitário declarado pelo comprador em `/compras/comprar`; sugere o custo no recebimento da OC |
...
| `uq_inv_sessao_idempotency` | `siso_inventario_sessoes` | `(idempotency_key) WHERE idempotency_key IS NOT NULL` | Dedup de duplo-clique em "Criar Sessão" — 1 sessão por `idempotency_key` (token do cliente, 1 por intenção de criação). **Substitui o limite P055** (`uq_inv_sessao_galpao_dia`, dropado em `20260608`): agora N sessões por galpão por dia são permitidas; só o duplo-clique acidental (mesma key) é deduplicado. | `20260608_inv_sessao_idempotency.sql` (P055-revert) |
...
**Vendas por janela (2026-06-19, `20260619_cobertura_vendas_7_30_60.sql`):** DROP+recria a MV no mesmo shape 3D, trocando o CTE `giro_30d` por um único scan de 60d com `SUM(quantidade) FILTER` por janela, **adicionando as colunas `vendas_7d`, `vendas_30d`, `vendas_60d`** (un de SAÍDA `S` nos últimos 7/30/60 dias). É **superset** — todas as colunas anteriores ficam (`giro_diario` continua `= vendas_30d / 30.0`, comportamento idêntico). Índices `uq_cobertura`/`idx_cobertura_status` + `wms_refresh_cobertura()` recriados; o cron `wms_refresh_cobertura_1min` (REFRESH CONCURRENTLY por nome) segue válido. ⚠️ **Nota (2026-06-19):** o chip de velocidade da tela de Compras NÃO usa estas colunas — venda de item esgotado/marketplace não gera mov `S`, então a tela calcula vendas 7/30/60 + giro a partir dos **PEDIDOS** (`siso_pedido_itens`), não do ledger. Estas colunas da MV ficam disponíveis mas hoje sem consumidor.
...
| 2026-06-05 | `20260607_inv_sessao_unique_galpao_dia.sql` | **Raio-X Fase 2 (P055)** — UNIQUE partial index `uq_inv_sessao_galpao_dia` em `siso_inventario_sessoes(galpao_id, (criado_em AT TIME ZONE 'UTC')::date) WHERE status<>'cancelada' AND continua=false`. No máx. 1 sessão não-cancelada/não-contínua por galpão por dia. **⚠ REVERTIDO em `20260608_inv_sessao_idempotency.sql`** — o limite por dia bloqueava criação legítima de múltiplas sessões. |
...
| 2026-06-19 | `20260619_cobertura_vendas_7_30_60.sql` | **Redesign Compras** — DROP+recria MV `siso_cobertura_estoque` (superset) adicionando `vendas_7d`/`vendas_30d`/`vendas_60d` (un vendidas reais por janela, scan único de 60d com `SUM(...) FILTER`). `giro_diario` mantém `= vendas_30d/30.0`. Índices + `wms_refresh_cobertura()` recriados. Alimenta o chip de velocidade da tela de Compras. |
...
| 2026-06-11 | `20260611g_guarda_liberacao_parcial_marca_indice.sql` | `wms_confirmar_guarda_atomico` + `wms_cancelar_pendencia_guarda_atomico`: L `liberacao_guarda` agora com `origem_detalhes.parcial=true` — 2ª liberação parcial na MESMA R deixava de colidir no `uq_mov_estorno_unico` (guarda parcial em 2+ locs estava quebrada). Proteção contra double-release segue na própria RPC (FOR UPDATE + LEAST recomputado). Fix EST-01. |
| 2026-06-11 | `20260611h_inventario_contagens_unique.sql` | Dedup + `UNIQUE INDEX uq_inventario_contagens_tripla_operador(sessao_id, localizacao_id, produto_id, contada_por)` em `siso_inventario_contagens` (mata check-then-act duplicado); `wms_contagem_inline_atomica` recriada como upsert incremental (ON CONFLICT DO UPDATE qty+=); `wms_metricas_localizacao` ordena por `contagem_finalizada_em DESC` (era uuid aleatório). Fix P2-INV-01/P3-16. |
| 2026-06-11 | `20260611l_custo_medio_estorno_parcial.sql` | Branch P110 de `wms_inserir_movimentacao`: estorno PARCIAL de entrada de custo agora usa remoção ponderada `(saldo_global*cm − qty*custo_orig)/(saldo_global − qty)` com guards; só o estorno TOTAL restaura `cm_anterior`. Fix P2-EST-01. |
| 2026-06-11 | `20260611m_system_user.sql` | Usuário 'Sistema (automação)' uuid-zero em `siso_usuarios` (ativo=false, pin `'----'` inlogável) — destrava o cron de transferências expiradas (FK `usuario_id` falhava em todo estorno). Fix P2-EST-02. |
...
| 2026-07-29 | `20260729091739_inventario_aplica_saldo_contado_live.sql` | **Inventário como balanço físico.** `wms_aplicar_sessao_inventario` deixa de executar o `delta` congelado como ordem fixa e, sob lock da tripla, reconcilia `saldo_live → qty_contada_final`. Snapshot/delta original ficam preservados na divergência e em `origem_detalhes`; saldo já igual vira divergência `aplicada` sem mov. `wms_estornar_sessao_inventario` passa a reabrir também essas linhas sem movimento. |
