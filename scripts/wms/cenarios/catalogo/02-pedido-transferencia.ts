...
export default {
  ...
  run: async (ctx, { sku }) => {
    ...
    // buscarLocComMaiorSaldoNoGalpao(SP_galpao, produtoWmsId) quando o
    // snapshot empresa-origem está vazio (NetAir sem saldo em CWB).
    // DEFAULT-PICKING também existe em SP (seed P6), então o fallback final
    // funciona mesmo sem live lookup.
