...
-- Envolve a mov de reconciliação (inventario_ganho/perda) + os inserts de
-- contagem/divergência + a marcação da loc/sessão numa única transação. A
-- versão anterior (registrarContagemInline v1) fazia a mov e depois 3 writes
-- separados: uma falha após a mov deixava ganho/perda sem registro de contagem.
BEGIN;
