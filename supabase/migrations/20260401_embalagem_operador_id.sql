-- Track which operator actually packed the order (may differ from who started separation).
-- Used for printer resolution: label prints on the packing operator's printer.

ALTER TABLE siso_pedidos
  ADD COLUMN IF NOT EXISTS embalagem_operador_id uuid REFERENCES siso_usuarios(id);

-- Update RPC to accept packing operator and store/return it instead of separacao_operador_id.
